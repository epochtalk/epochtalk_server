defmodule EpochtalkServerWeb.MentionView do
  ...
  """
  def render("page.json", %{mentions: mentions, pagination_data: %{page: page, limit: limit, next: next, prev: prev}, extended: extended}) do
    mentions = Enum.reduce(mentions, [], fn m, acc ->
      formatted_mention = %{
      ...
      if extended,
        do: [formatted_mention
        |> Map.put(:body_html, m.post.content["body_html"] || m.post.content["body"])
        |> Map.put(:board_name, m.thread.board.name)
        |> Map.put(:board_id, m.thread.board_id) | acc],
        else: [formatted_mention | acc] # prepend to accumulator list
    end) |> Enum.reverse() # reverse, because of prepend to accumulator
    result = %{
      data: mentions,
      next: next,
      prev: prev,
